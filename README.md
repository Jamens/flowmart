...
- **当前权限粒度**：购物车、订单创建、地址归属均严格取自令牌用户；但**订单列表与流转动作目前对登录用户全开放**（管理后台视角，便于演示推动任意订单），细粒度「仅买家/管理员可操作」属于后续 RBAC 工作（见待办）。`GET /users/{id}` 不返回收货地址，地址须经归属校验的 `/users/{id}/addresses` 获取。
