...
class LoginRateLimiter:
    ...
    def _client_ip(request) -> str:
        ...
        # 注意：若代理不可信，攻击者可伪造 X-Forwarded-For 绕过——生产应在网关层
        # 用 `X-Forwarded-For` 最右可信跳覆盖，这里仅做「有则取第一个」的通用处理。
    ...
    def retry_after(self, request, username: str) -> int:
        ...
        remaining = settings.LOGIN_RATE_LIMIT_WINDOW - (time.monotonic() - b.window_start)
